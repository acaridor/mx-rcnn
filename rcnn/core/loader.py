...
class AnchorLoader(mx.io.DataIter):
    ...
    def infer_shape(self, max_data_shape=None, max_label_shape=None):
        ...
        label = assign_anchor(feat_shape[0], np.zeros((0, 5)), im_info[0],
    ...
    def get_batch(self):
        ...
        for data, label in zip(data_list, label_list):
            ...
            label = assign_anchor(feat_shape, label['gt_boxes'], data['im_info'][0],
                                  self.feat_stride, self.anchor_scales,
                                  self.anchor_ratios, self.allowed_border)
            new_label_list.append(dict(zip(['label', 'bbox_target', 'bbox_weight'], label)))
...
class DetLoader(mx.io.DataIter):
    def __init__(self, roidb, batch_size=1, shuffle=False, ctx=None, work_load_list=None, aspect_grouping=False):
        """
        This Iter will provide roi data to Fast R-CNN network
        :param roidb: must be preprocessed
        :param batch_size: must divide BATCH_SIZE(128)
    ...
    def get_batch(self):
        ...
        # get training data for multigpu
        data_list = []
        label_list = []
        for islice in slices:
            iroidb = [roidb[i] for i in range(islice.start, islice.stop)]
            ...
            label['gt_boxes'] = label['gt_boxes'][np.newaxis, :, :]
